...
class DownloadsPage:
    ...
    def build(self):
        ...
        ffmpeg_hint = QLabel(
            self._text(
                (
                    f"ffmpeg 状态：已就绪 ({ffmpeg_status.ffmpeg_path})"
                    if ffmpeg_status.available
                    else "ffmpeg 状态：未检测到，音视频同时选择时会自动改为分开下载。"
                ),
                (
                    f"ffmpeg: ready ({ffmpeg_status.ffmpeg_path})"
                    if ffmpeg_status.available
                    else "ffmpeg: not found, combined selections will fall back to separate downloads."
                ),
            )
        )
        ...
        input_group = QGroupBox(self._text("单视频下载", "Single Video Download"))
        input_layout = QFormLayout(input_group)
        input_layout.setSpacing(10)

        self.value_input = QLineEdit()
        self.value_input.setPlaceholderText(
            self._text("输入 BV 号或 B 站视频链接", "Enter a BV id or bilibili video URL")
        )
        ...
        self.name_input.setPlaceholderText(
            self._text(
                "可选显示名称，不填时默认使用解析结果",
                "Optional display name, defaults to the detected title",
            )
        )
        self.name_input.textChanged.connect(self._update_selection_summary)

        self.manual_format_input = QLineEdit()
        self.manual_format_input.setPlaceholderText(
            self._text(
                "可选：手动填写 yt-dlp 格式选择器；留空则使用下方可视化选择",
                "Optional: enter a yt-dlp format selector; leave blank to use the visual picker below",
            )
        )
        self.manual_format_input.textChanged.connect(self._update_selection_summary)

        inspect_button = QPushButton(self._text("查看可用格式", "Inspect Formats"))
        inspect_button.clicked.connect(self._start_format_probe)

        start_button = QPushButton(self._text("开始下载", "Start Download"))
        start_button.clicked.connect(self._start_single_video_download)

        refresh_button = QPushButton(self._text("刷新页面", "Refresh"))
        refresh_button.clicked.connect(self.refresh)

        buttons_layout = QHBoxLayout()
        buttons_layout.setSpacing(8)
        buttons_layout.addWidget(inspect_button)
        ...
        input_layout.addRow(self._text("高级格式覆盖", "Advanced Format Override"), self.manual_format_input)
        input_layout.addRow(buttons_layout)
        input_layout.addRow(self._text("状态", "Status"), self.status_label)
        layout.addWidget(input_group)

        self.formats_title_label = QLabel(
            self._text(
                "<b>当前格式结果：</b> 还没有进行探测。",
                "<b>Current formats:</b> nothing has been inspected yet.",
            )
        )
        ...
        self.video_table.setHorizontalHeaderLabels(
            self._text(
                ["格式ID", "清晰度", "说明", "视频编码", "大小"],
                ["Format ID", "Quality", "Label", "Video Codec", "Size"],
            )
        )
        ...
        self.audio_table.setHorizontalHeaderLabels(
            self._text(
                ["格式ID", "音频类型", "说明", "音频编码", "大小"],
                ["Format ID", "Audio Type", "Label", "Audio Codec", "Size"],
            )
        )
        ...
        options_group = QGroupBox(self._text("输出选项", "Output Options"))
        ...
        if not ffmpeg_status.available:
            self.merge_checkbox.setToolTip(
                self._text(
                    "当前环境没有 ffmpeg，所以只能分开下载音视频。",
                    "ffmpeg is not available, so video and audio can only be downloaded separately.",
                )
            )
        self.merge_checkbox.toggled.connect(self._update_selection_summary)

        self.selection_summary_label = QLabel(
            self._text("下载计划：默认使用自动选择。", "Plan: the default automatic selector will be used.")
        )
        ...
        tasks_group = QGroupBox(self._text("下载任务", "Download Tasks"))
        tasks_layout = QVBoxLayout(tasks_group)
        self.tasks_table = QTableWidget(0, 7)
        self.tasks_table.setHorizontalHeaderLabels(
            self._text(
                ["ID", "名称", "状态", "进度", "来源", "文件路径", "错误"],
                ["ID", "Name", "Status", "Progress", "Source", "File Path", "Error"],
            )
        )
        self._configure_table(self.tasks_table, header_stretch_column=4)
    ...
    def _refresh_tasks_table(self) -> None:
        ...
        for row_index, task in enumerate(tasks):
            self.tasks_table.setItem(row_index, 0, table_item(str(task.id)))
            self.tasks_table.setItem(row_index, 1, table_item(task.display_name))
            self.tasks_table.setItem(row_index, 2, table_item(task.status.value))
            self.tasks_table.setItem(row_index, 3, table_item(f"{task.progress:.0f}%"))
            self.tasks_table.setItem(row_index, 4, table_item(task.source_url))
            self.tasks_table.setItem(row_index, 5, table_item(task.file_path or "-"))
            self.tasks_table.setItem(row_index, 6, table_item(task.error_message or "-"))
